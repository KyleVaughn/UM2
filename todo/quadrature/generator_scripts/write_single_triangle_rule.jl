...
const m_all = SVector(1, 2, 3, 4, 5, 7, 8, 10, 12, 14, 16, 19, 21, 24, 27, 30, 33, 37, 40,
                      44)
const m = m_all[p]

# Polar moments from Table I.
const ν_all = SVector(+1 // 1,
                      +1 // 4,
                      -1 // 10,
                      +1 // 10,
                      -2 // 35,
...
const iν_all = SMatrix{44, 2, Int64, 88}(0, 2, 3, 4, 5, 6, 6, 7, 8, 8, 9, 9, 10, 10, 11, 11,
                                         12, 12, 12, 13, 13, 14, 14, 14, 15, 15, 15, 16, 16,
                                         16, 17, 17, 17, 18, 18, 18, 18, 19, 19, 19, 20, 20,
                                         20, 20,
                                         0, 0, 3, 0, 3, 0, 6, 3, 0, 6, 3, 9, 0, 6, 3, 9, 0,
                                         6, 12, 3, 9, 0, 6, 12, 3, 9, 15, 0, 6, 12, 3, 9,
                                         15, 0, 6, 12, 18, 3, 9, 15, 0, 6, 12, 18)
...
function area(x)
    ...
    return [(w[i], α[i], β[i], γ[i]) for i in 1:nsum]
end
...
println(io, "                       )")
println(io,
        """    points = SVector(:(\$(tuple(T(big"$(points[1][1])"), T(big"$(points[1][2])")))),""")
for i in 2:ng
    println(io,
            """                     :(\$(tuple(T(big"$(points[i][1])"), T(big"$(points[i][2])")))),""")
end
println(io, "                      )")
