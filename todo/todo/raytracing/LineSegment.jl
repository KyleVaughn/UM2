...
function intersect_edges_CUDA(lines::Vector{LineSegment{2, T}},
    ...
    intersection_array_gpu = CUDA.fill(Point2D{T}(NaN, NaN), ceil(Int64, 2sqrt(nedges)),
                                       nlines)
    ...
    intersection_points = [filter!(x -> !isnan(x[1]),
                                   collect(intersection_array[:, i])) for i in 1:nlines]
    Threads.@threads for i in 1:nlines
        sort_intersection_points!(lines[i], intersection_points[i])
    end
    return intersection_points
...
function findface(p::Point, faces::Vector{<:Face})
    @inbounds for i in 1:length(faces)
        p ∈ faces[i] && return i
    end
    return 0
end
