...
main(int argc, char ** argv) -> int
{
  ...
  if (argc != 4) {
    ...
    um2::logger::error("Usage: ", exec_name, " <uo2_size> <clad_size> <h2o_size>");
    return 1;
  }

  char * end = nullptr;
  Float const uo2_size = um2::strto<Float>(argv[1], &end);
  ASSERT(end != nullptr);
  ASSERT(uo2_size > 0.0);
  end = nullptr;

  Float const clad_size = um2::strto<Float>(argv[2], &end);
  ASSERT(end != nullptr);
  ASSERT(clad_size > 0.0);
  end = nullptr;

  Float const h2o_size = um2::strto<Float>(argv[3], &end);
  ASSERT(end != nullptr);
  ASSERT(h2o_size > 0.0);
  end = nullptr;

  um2::logger::info("UO2 size: ", uo2_size);
  um2::logger::info("Clad size: ", clad_size);
  um2::logger::info("H2O size: ", h2o_size);
  ...
  std::vector<std::string> const groups = {"Material_Fuel", "Material_Clad",
                                           "Material_Moderator"};
  std::vector<double> const sizes = {uo2_size, clad_size, h2o_size};
  int const mesh_output =
      um2::gmsh::model::mesh::setMeshFieldFromGroups(2, groups, sizes);
  if (mesh_output < 0) {
    um2::logger::error("Failed to set mesh field from groups");
    return 1;
  }
