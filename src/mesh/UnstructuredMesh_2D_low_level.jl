...
#
## Area of a triangle
#function area(mesh::UnstructuredMesh_2D{F, U}, face::NTuple{4, U}) where {F <: AbstractFloat, U <: Unsigned}
#    return area(materialize_face(mesh, face))
#end
#
## Area of a quadrilateral
#function area(mesh::UnstructuredMesh_2D{F, U}, face::NTuple{5, U}) where {F <: AbstractFloat, U <: Unsigned}
#    return area(materialize_face(mesh, face))
#end
#
## Area of a quadratic triangle
#function area(mesh::UnstructuredMesh_2D{F, U}, face::NTuple{7, U}) where {F <: AbstractFloat, U <: Unsigned}
#    return area(materialize_face(mesh, face))
#end
#
## Area of a quadratic quadrilateral
#function area(mesh::UnstructuredMesh_2D{F, U}, face::NTuple{9, U}) where {F <: AbstractFloat, U <: Unsigned}
#    return area(materialize_face(mesh, face))
#end
#
...
# Tuple of MVectors of point IDs representing the 3 edges of a triangle
# @code_warntype checked 2021/11/22
function edges(face::NTuple{4, U}) where {U <: Unsigned}
    edges = ( MVector{2, U}(face[2], face[3]),
...
# Tuple of MVectors of point IDs representing the 4 edges of a quadrilateral
# @code_warntype checked 2021/11/22
...
# Tuple of MVectors of point IDs representing the 3 edges of a quadratic triangle
# @code_warntype checked 2021/11/22
function edges(face::NTuple{7, U}) where {U <: Unsigned}
    edges = ( MVector{3, U}(face[2], face[3], face[5]),
...
# Tuple of MVectors of point IDs representing the 4 edges of a quadratic quadrilateral
# @code_warntype checked 2021/11/22
function edges(face::NTuple{9, U}) where {U <: Unsigned}
    edges = ( MVector{3, U}(face[2], face[3], face[6]),
...
function edges(faces::Vector{<:Tuple{Vararg{U, N} where N}}) where {U <: Unsigned}
    edge_arr = edges.(faces)
    edges_unfiltered = [ edge for edge_vec in edge_arr for edge in edge_vec ]
    # Filter the duplicate edges
    edges_filtered = sort(unique(edges_unfiltered))
...
#function materialize_edge(points::Vector{Point_2D{F}},
#                          edge::NTuple{2, U}) where {F <: AbstractFloat, U <: Unsigned}
#    return LineSegment_2D(edge_points(mesh, edge))
#end
#
## Return a QuadraticSegment_2D from the point IDs in an edge
#function materialize_edge(mesh::UnstructuredMesh_2D{F, U},
#                          edge::NTuple{3, U}) where {F <: AbstractFloat, U <: Unsigned}
