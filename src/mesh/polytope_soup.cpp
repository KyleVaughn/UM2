...
PolytopeSoup::PolytopeSoup(String const & filename)
{
  read(filename);
}
...
void
PolytopeSoup::getElement(Size const i, VTKElemType & type, Vector<I> & conn) const
{
...
auto
PolytopeSoup::getElementBoundingBox(Size const i) const -> AxisAlignedBox3
{
  LOG_TRACE("PolytopeSoup::getElementBoundingBox(i = " + toString(i) + ")");
  ASSERT(i < _element_types.size());
  ...
  case VTKElemType::QuadraticQuad: {
    ...
    QuadraticQuadrilateral2 const quad8(p2[0], p2[1], p2[2], p2[3], p2[4], p2[5],
                                           p2[6], p2[7]);
...
auto
PolytopeSoup::getElementCentroid(Size const i) const -> Point3
{
...
void
PolytopeSoup::getElset(Size const i, String & name, Vector<I> & ids,
                             Vector<F> & data) const
{
...
auto
PolytopeSoup::addVertex(F x, F y, F z) -> Size
{
...
auto
PolytopeSoup::addVertex(Point3 const & p) -> Size
{
...
auto
PolytopeSoup::addElement(VTKElemType const type, Vector<I> const & conn) -> Size
{
...
auto
PolytopeSoup::addElset(String const & name, Vector<I> const & ids, Vector<F> data)
    -> Size
{
...
PolytopeSoup::mortonSortElements()
{
  ...
  Vector<Size> inv_perm(num_elems);
...
PolytopeSoup::sortElsets()
{
  ...
  for (Size i = 0; i < num_elsets; ++i) {
    elset_name_index_pairs[i] =
        um2::make_pair<String, I>(_elset_names[i], i);
  }
...
PolytopeSoup::getSubmesh(String const & elset_name,
                               PolytopeSoup & submesh) const
{
  ...
  for (auto const & type_count : submesh_elem_type_counts) {
    ...
    Size const count = type_count.second;
  ...
  for (Size i = 0; i < num_elsets; ++i) {
    ...
    for (Size j = 0; j < num_ids; ++j) {
      I const old_element_id = intersection[static_cast<size_t>(j)];
      auto const * const it = std::lower_bound(elset_ids_begin, elset_ids_end, old_element_id);
      ASSERT(*it == old_element_id);
...
void
PolytopeSoup::reserveMoreElements(VTKElemType const elem_type, Size const num_elems)
{
  // Element types
  ...
  Size const verts_per_elem = verticesPerElem(elem_type);
...
void
PolytopeSoup::reserveMoreVertices(Size const num_verts)
{
...
static void
addNodesToMesh(PolytopeSoup & mesh, Size const num_verts, Size const num_dimensions,
                             ...
                             String const & h5filename, PolytopeSoup & soup)
{
  ...
  std::string const dimensions = xdataitem.attribute("Dimensions").value();
  if (datatype_size == 4) {
    addElementsToMesh<int32_t>(num_elements, topology_type, dimensions, soup,
                                     dataset, datatype);
  } else if (datatype_size == 8) {
    addElementsToMesh<int64_t>(num_elements, topology_type, dimensions, soup,
                                     dataset, datatype);
  } else {
...
template <std::signed_integral T>
static void
addElsetToMesh(PolytopeSoup & soup, Size const num_elements,
               H5::DataSet const & dataset, H5::IntType const & datatype,
