                                 ...
                                 Float const y, Int * const face_id)
{
  ...
  case um2::MeshType::Tri: 
  {
    *face_id = sp.getTriMesh(cc.mesh_id).faceContaining(um2::Point2(x, y));
    break;
  }
  case um2::MeshType::Quad: 
  {
    *face_id = sp.getQuadMesh(cc.mesh_id).faceContaining(um2::Point2(x, y));
    break;
  }
  case um2::MeshType::QuadraticTri: {
    *face_id = sp.getTri6Mesh(cc.mesh_id).faceContaining(um2::Point2(x, y));
    break;
  }
  case um2::MeshType::QuadraticQuad: {
    *face_id = sp.getQuad8Mesh(cc.mesh_id).faceContaining(um2::Point2(x, y));
    break;
  }
  default:
    LOG_ERROR("Invalid mesh type");
                            ...
                            Int * const n)
{
  auto & sp = *reinterpret_cast<um2::mpact::Model *>(model);
  auto const & cc = sp.getCoarseCell(cc_id);
  um2::Ray2 const ray(um2::Point2(origin_x, origin_y),
