...
"""
function safe_add_physical_group(name::String,
                                 entities::Vector{NTuple{2, I}}) where {I <: Integer}
    dims = unique!(getindex.(entities, 1))
    for dim in dims
        current_groups = gmsh.model.get_physical_groups(dim)
        current_names = [gmsh.model.get_physical_name(dim, grp[2])
        ...
        if name ∈ current_names
            ...
            p = gmsh.model.add_physical_group(dim,
                                              [ent[2]
                                               for ent in filter(e -> e[1] == dim, entities)],
                                              tag)
        else
            p = gmsh.model.add_physical_group(dim,
                                              [ent[2]
