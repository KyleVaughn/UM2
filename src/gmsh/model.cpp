...
//         if (dim != dim_max) { continue; }
//         int const tag = dimtag.second;
//         std::string name;
//         gmsh::model::getPhysicalName(dim, tag, name);
//         if (name.starts_with("Material ")) {
//             auto const it = std::find(material_names.begin(), material_names.end(),
//             name); if (it != material_names.end() && *it == name) {
...
//             int const tag,
//             bool const remove_object,
//             bool const remove_tool)
//     {
...
//         }
//         int const model_dim = object_dim;
//
...
//         gmsh::model::getPhysicalGroups(pre_dimtags);
//         for (auto const & dimtag : pre_dimtags) {
//             int const dim = dimtag.first;
//             if (dim != model_dim) { continue; }
...
//             pre_physical_group_ent_tags.insert(pre_physical_group_ent_tags.begin() +
//             idx, tags);
//         }
...
//         if (remove_object) {
//             gmsh::model::removeEntities(object_dimtags, true);
//         }
//         if (remove_tool) {
//             gmsh::model::removeEntities(tool_dimtags, true);
//         }
...
//         //              Add the fragment children of the entity to the new physical
//         group.
//         //          Else,
//         //              Add the entity to the new physical group.
//         //  Add the new physical groups.
//         //
...
//         post_physical_group_ent_tags(physical_group_names.size()); for (size_t i = 0; i
//         < physical_group_names.size(); ++i) {
...
//                 }
//                 if (found) {
//                     // The entity is in the object or tool, so add the fragment
//                     children
//                     // to the new physical group.
//                     for (auto const & child : out_dimtags_map[idx]) {
//                         auto const child_it = std::lower_bound(
//                                 new_tags.begin(),
//                                 new_tags.end(),
//                                 child.second);
...
//                     }
//                 } else {
//                     // The entity is not in the object or tool, so add the entity to
//                     the
//                     // new physical group.
//                     auto const child_it = std::lower_bound(
//                             new_tags.begin(),
...
//             // Insert the physical group, keeping the vector sorted
//             // by name
...
//                 std::string const & mat_name = "Material " + to_string(mat.name);
//                 auto const it = std::lower_bound(physical_group_names.begin(),
//                                                  physical_group_names.end(),
//                                                  mat_name);
//                 if (it == physical_group_names.end() || *it != mat_name) {
//                     Log::warn("'Material " + to_string(mat.name) + "' not found in
//                     model");
//                 } else {
//                     mat_indices[i] = static_cast<size_t>(it -
//                     physical_group_names.begin());
//                 }
//             }
//             for (size_t i = 1; i < nmats; ++i) {
//                 if (mat_indices[i] == guard) { continue; }
//                 std::vector<int> & mat_i =
//                 post_physical_group_ent_tags[mat_indices[i]]; for (size_t j = 0; j < i;
//                 ++j) {
//                     if (mat_indices[j] == guard) { continue; }
//                     std::vector<int> & mat_j =
//                     post_physical_group_ent_tags[mat_indices[j]]; std::vector<int>
//                     intersection;
//                     // We anticipate that the intersection will be empty, so we don't
//                     // start with a set_difference.
//                     std::set_intersection(
//                             mat_i.begin(), mat_i.end(),
//                             mat_j.begin(), mat_j.end(),
...
//                     post_physical_group_ent_tags[i],// Tags of the entities in the
//                     group physical_group_tag[i],          // Old tag of the physical
//                     group physical_group_names[i]);       // Name of the physical group
//                 UM2_ASSERT(pgroup_tag == physical_group_tag[i],
//                         "Old tag of physical group does not match new tag");
//         }
//
...
//             prev_loop_tag}); out_tags.emplace_back(annulus_tag); prev_loop_tag =
//             loop_tag;
//         }
//         gmsh::model::occ::synchronize();
//         // Add materials
//         for (size_t i = 0; i < nradii; ++i) {
//             add_to_physical_group(2, {out_tags[i]}, -1, "Material " +
//             to_string(materials[i].name));
//             // Color entities according to materials
//             Color const color = materials[i].color;
//             gmsh::model::setColor(
//                     {{2, out_tags[i]}}, // Entities to color
//                     static_cast<int>(color.r),
//                     static_cast<int>(color.g),
//                     static_cast<int>(color.b),
//                     static_cast<int>(color.a),
...
//         // Reverse the ordering of the rows of pin_ids, so the vector ordering matches
//         the
//         // spatial ordering.
...
//                         double const x = 0.5 * (x_min(box) + x_max(box)) + offset[0u];
//                         double const y = 0.5 * (y_min(box) + y_max(box)) + offset[1u];
//                         // Do the innermost disk
//                         double const r0 = radii[pin_id][0];
//                         int const circle0_tag = gmsh::model::occ::addCircle(x, y, 0.0,
...
//                         for (size_t k = 1; k < nrad; ++k) {
//                             double const r = radii[pin_id][k];
//                             int const circle_tag = gmsh::model::occ::addCircle(x, y, z,
//                             r); int const loop_tag =
//                             gmsh::model::occ::addCurveLoop({circle_tag}); int const
//                             annulus_tag =
//                                 gmsh::model::occ::addPlaneSurface({loop_tag,
//                                 prev_loop_tag});
