...
void
SpatialPartition::checkMeshExists(MeshType mesh_type, Size mesh_id) const
{
  switch (mesh_type) {
  case MeshType::Tri:
...
auto
SpatialPartition::addMaterial(Material const & material) -> Size
{
                                               ...
                                               Size const mesh_order) -> Size
{
  ...
  F constexpr big_eps = 1e-4;
#else
  F constexpr eps = 1e-6f;
  F constexpr big_eps = 1e-4f;
#endif
  ...
  F const sum_ring_areas =
      std::reduce(ring_areas.begin(), ring_areas.end(), static_cast<F>(0));
...
auto
SpatialPartition::makeRectangularPinMesh(Vec2<F> dxdy, Size nx, Size ny) -> Size
{
  ...
  Size const mesh_id = _quads.size();
...
auto
SpatialPartition::makeCoarseCell(Vec2<F> const dxdy, MeshType const mesh_type,
                                 Size const mesh_id,
...
//// auto
//// SpatialPartition::stdMakeLattice(std::vector<std::vector<Size>> const & rtm_ids)
////    -> Size
////{
...
// template <std::floating_point T, std::integral I>
// auto
// SpatialPartition::stdMakeCore(std::vector<std::vector<Size>> const & asy_ids)
//    -> Size
//{
//  // Convert to um2::Vector
