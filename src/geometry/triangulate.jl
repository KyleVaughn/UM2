...
function triangulate(poly::Polygon{N, 2, T}) where {N, T}
    ...
    return Vec(triangles.data)
end

"""
    triangulate(poly::Polygon{N, 2, BigFloat}) where {N}

Return a Vector of the `N`-2 triangles that partition the 2D `Polygon`. 
...
"""
    triangulate(quad::Quadrilateral{3,T}, ::Val{N}) where {N,T}

Return a Vec of (2`N`+2)^2 triangles that approximately partition the `Quadrilateral`. 
"""
function triangulate(quad::Quadrilateral{3,T}, ::Val{N}) where {T, N}
    # N is the number of divisions of each edge
    N1 = N + 1
    inv_N1 = T(1)/N1
    triangles = MVector{2N1^2, Triangle{3,T}}(undef)
    if N === 0
...
"""
    triangulate(quad::Quadrilateral{3,BigFloat}, ::Val{N}) where {N}

Return a Vector of (2`N`+2)^2 triangles that approximately partition the `Quadrilateral3D`. 
"""
function triangulate(quad::Quadrilateral{3,BigFloat}, ::Val{N}) where {N}
    # N is the number of divisions of each edge
    N1 = N + 1
    inv_N1 = BigFloat(1)/N1
    triangles = Vector{Triangle{3,BigFloat}}(undef, 2N1^2)
    if N === 0
        triangles[1] = Triangle(quad[1], quad[2], quad[3])
        triangles[2] = Triangle(quad[3], quad[4], quad[1])
    else
...
Return a Vector of (2`N`+2)^2 triangles that approximately partition the `QuadraticTriangle`. 
"""
...
Return a Vector of (2`N`+2)^2 triangles that approximately partition 
the `QuadraticQuadrilateral`. 
"""
