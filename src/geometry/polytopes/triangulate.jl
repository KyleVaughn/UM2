...
"""
    triangulate(tri6::QuadraticTriangle{Point{Dim,T}}, ::Val{N}) where {Dim,T,N}

Return a Vec of (2`N`+2)^2 triangles that approximately partition the `QuadraticTriangle`. 
"""
@generated function triangulate(tri6::QuadraticTriangle{Point{Dim,T}}, ::Val{N}) where {Dim,T,N}
    # N is the number of divisions of each edge
    N1 = N + 1
    inv_N1 = T(1)/N1
    if N === 0
    ...
    return quote
        return Vec{$(N1^2), Triangle{Point{$Dim,$T}}}(tuple($(exprs...)))
    end
end

"""
    triangulate(quad::QuadraticQuadrilateral{Point{Dim,T}}, ::Val{N}) where {Dim,T,N}

Return a Vec of (2`N`+2)^2 triangles that approximately partition the `QuadraticQuadrilateral`. 
"""
@generated function triangulate(quad8::QuadraticQuadrilateral{Point{Dim,T}}, 
                                ::Val{N}) where {Dim,T,N}
    # N is the number of divisions of each edge
    N1 = N + 1
    inv_N1 = T(1)/N1
