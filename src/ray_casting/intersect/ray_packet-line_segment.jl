export ray_packet_line_segment_intersection
...
function ray_packet_line_segment_intersection(R::Ray2Packet{N, T}, 
                                              P1::Point2{T},
                                              P2::Point2{T}) where {N, T}
    L1x = ntuple(i -> P1[1], Val(N))
    L1y = ntuple(i -> P1[2], Val(N))
    L2x = ntuple(i -> P2[1], Val(N))
    L2y = ntuple(i -> P2[2], Val(N))
    # 𝘃 = L[2] - L[1]
    𝘃x = @. L2x - L1x
    𝘃y = @. L2y - L1y
    # 𝘂 = R.origin - L[1]
