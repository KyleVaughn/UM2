...
TEST_CASE(makeAssembly_2d)
{
  ...
  ASSERT_NEAR(grid.divs[0][0], -1, static_cast<T>(1e-6));
  ASSERT_NEAR(grid.divs[0][1], 1, static_cast<T>(1e-6));
}
...
TEST_CASE(makeCore)
{
  ...
  ASSERT_NEAR(model.core.grid.divs[0][0], 0, static_cast<T>(1e-6));
  ASSERT_NEAR(model.core.grid.divs[0][1], 2, static_cast<T>(1e-6));
  ASSERT_NEAR(model.core.grid.divs[0][2], 4, static_cast<T>(1e-6));
  ASSERT_NEAR(model.core.grid.divs[1][0], 0, static_cast<T>(1e-6));
  ASSERT_NEAR(model.core.grid.divs[1][1], 1, static_cast<T>(1e-6));
  ASSERT_NEAR(model.core.grid.divs[1][2], 2, static_cast<T>(1e-6));
}
//
// template <typename T, typename I>
// TEST_CASE(test_import_coarse_cells)
...
// model.import_coarse_cells("./test/mpact/mesh_files/coarse_cells.inp");
// std::string filepath =
//     std::filesystem::current_path().string() + "/mpact_export_test/model.xdmf";
...
//     2, um2::Point2<T>(static_cast<T>(0.5), static_cast<T>(0.5)));
// ASSERT(face_id == 0, "face_id");
// face_id = -2;
// face_id = model.coarse_cell_find_face(
//     2, um2::Point2<T>(static_cast<T>(0.5), static_cast<T>(1.5)));
// ASSERT(face_id == -1, "face_id");
// face_id = -2;
//
// face_id = model.coarse_cell_find_face(
//     1, um2::Point2<T>(static_cast<T>(0.5), static_cast<T>(0.05)));
// ASSERT(face_id == 0, "face_id");
// face_id = -2;
// face_id = model.coarse_cell_find_face(
//     1, um2::Point2<T>(static_cast<T>(0.5), static_cast<T>(-0.05)));
...
// face_id = -2;
// face_id = model.coarse_cell_find_face(
//     1, um2::Point2<T>(static_cast<T>(0.5), static_cast<T>(0.95)));
// ASSERT(face_id == 1, "face_id");
// face_id = -2;
// END_TEST_CASE
//
...
TEST_SUITE(SpatialPartition)
{
  ...
  //    TEST_CASE("export_mesh", (test_export_mesh<T, I>));
  ...
  //    TEST_CASE("coarse_cell_ray_intersect", (test_coarse_cell_ray_intersect<T, I>));
}

auto
main() -> int
{
  RUN_SUITE((SpatialPartition<float, int16_t>));
  RUN_SUITE((SpatialPartition<float, int32_t>));
  RUN_SUITE((SpatialPartition<float, int64_t>));
  RUN_SUITE((SpatialPartition<double, int16_t>));
  RUN_SUITE((SpatialPartition<double, int32_t>));
  RUN_SUITE((SpatialPartition<double, int64_t>));
  return 0;
}
