...
TEST_CASE(sortElsets)
{
  ...
  ASSERT(elset_data == um2::Vector<F>({10, 2}));
}
...
TEST_CASE(mortonSortElements)
{
  ...
  F const zero = condCast<F>(0);
  F const third = condCast<F>(1) / condCast<F>(3);
  F const two_thirds = condCast<F>(2) / condCast<F>(3);
  ...
  p = um2::Point3(condCast<F>(1.5), condCast<F>(0.5), zero);
  ASSERT(type == um2::VTKElemType::Quad);
  ...
  p = um2::Point3(condCast<F>(0.5), condCast<F>(1.5), zero);
  ASSERT(type == um2::VTKElemType::Quad);
...
TEST_CASE(getSubmesh)
{
  ...
  ASSERT_NEAR(elset_data[0], 2, condCast<F>(1e-6));
