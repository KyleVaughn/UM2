...
void write_xdmf_file(std::string const & path, mpact::SpatialPartition<T, I> & model)
{
    ...
    std::string core_mn_str = std::to_string(nyass) + " x " +
                              std::to_string(nxass);
    ...
    for (length_t ixass = 0; ixass < nxass; ++ixass) {
        ...
        ss.str("");
        ss << "Assembly_" << std::setw(5) << std::setfill('0') << ass_id
           <<         "_" << std::setw(5) << std::setfill('0') << ass_id_ctr;
        ...
        for (length_t izlat = 0; izlat < nzlat; ++izlat) {
            ...
            ss.str("");
            ss << "Lattice_" << std::setw(5) << std::setfill('0') << lat_id
               <<        "_" << std::setw(5) << std::setfill('0') << lat_id_ctr;
            ...
            std::string z_values = std::to_string(low_z) + ", " +
                                   std::to_string(cut_z) + ", " +
...
//            T const dz = (high_z - low_z) / 2;
            ...
            std::string lat_mn_str = std::to_string(nyrtm) + " x " +
                                     std::to_string(nxrtm);
            ...
            for (length_t ixrtm = 0; ixrtm < nxrtm; ++ixrtm) {
                ...
                ss.str("");
                ss << "RTM_" << std::setw(5) << std::setfill('0') << rtm_id
                   <<     "_" << std::setw(5) << std::setfill('0') << rtm_id_ctr;
                ...
                std::string rtm_mn_str = std::to_string(nycells) + " x " +
                                         std::to_string(nxcells);
                ...
                for (length_t ixcell = 0; ixcell < nxcells; ++ixcell) {
                    ...
                    switch (mesh_type) {
                        ...
                        case 4: // Quadratic Triangle
                            model.quadratic_tri[static_cast<length_t>(mesh_id)].to_mesh_file(mesh_file);
                            break;
                        case 5: // Quadratic Quadrilateral
                            model.quadratic_quad[static_cast<length_t>(mesh_id)].to_mesh_file(mesh_file);
                            break;
                        case 6: // Quadratic Tri/Quad
                            model.quadratic_tri_quad[static_cast<length_t>(mesh_id)].to_mesh_file(mesh_file);
                            break;
                        default:
                    ...
                    write_xdmf_uniform_grid(xrtm_grid, h5file, h5filename,
                                            h5rtm_grouppath, mesh_file, mat_names_short);
                } // cell
                } // cell
...
template <std::floating_point T, std::signed_integral I>
void export_mesh(std::string const & path, mpact::SpatialPartition<T, I> & model)
{
...
static inline void map_lattice_idx_to_j_i(int const idx, int & j, int & i, int const M, int const N)
{
    j = M - idx / N - 1;
    i = idx % N;
}
...
template <std::floating_point T, std::signed_integral I>
void import_mpact_model(std::string const & path, mpact::SpatialPartition<T, I> & model)
{
    ...
    // Open the HDF5 file
    size_t const h5filepath_end = path.find_last_of("/") + 1;
    std::string h5filename = path.substr(
            h5filepath_end,
            path.size() - 4 - h5filepath_end) + "h5";
    std::string h5filepath = path.substr(0, h5filepath_end);
    um2::Log::debug("H5 filename: " + h5filename);
    ...
    // -------------------------------
    // Constraints:
    // - We want to use the SpatialPartition.make_core, make_assembly, make_lattice, make_rtm,
    //      and make_coarse_cell methods to create the model in order to avoid duplicating code.
    //      These functions take children IDs as arguments.
    // - We have to create ID 1 before ID 2, ID 2 before ID 3, etc. since the make_* methods
    //      do not take its own ID as an argument. Therefore we have to construct the model in a
    //      bottom-up fashion, i.e. we have to create the coarse cells before we can create the RTMs, etc.
    ...
    //       If this is the first lattice write the top and bottom Z positions to assembly_lattice_zs
    //       Else write the top Z position to assembly_lattice_zs
    ...
    for (auto const & assembly_node : xcore.children("Grid")) {
        ...
        // If the assembly ID is not in assembly_ids
        auto assembly_id_it = std::lower_bound(assembly_ids.begin(), assembly_ids.end(), assembly_id);
        if (assembly_id_it == assembly_ids.end() || *assembly_id_it != assembly_id) {
            ...
            // Allocate assembly_lattice_zs to M + 1
            assembly_lattice_zs.insert(assembly_lattice_zs.begin() + assembly_id_idx, vdouble());
            ...
            for (auto const & lattice_node : assembly_node.children("Grid")) {
                ...
                // If the lattice ID is not in lattice_ids
                auto lattice_id_it = std::lower_bound(lattice_ids.begin(), lattice_ids.end(), lattice_id);
                if (lattice_id_it == lattice_ids.end() || *lattice_id_it != lattice_id) {
                    ...
                    {
                        pugi::xml_node xlattice_mn = lattice_node.child("Information").next_sibling("Information");
                    ...
                    for (auto const & rtm_node : lattice_node.children("Grid")) {
                        ...
                        if (rtm_id_it == rtm_ids.end() || *rtm_id_it != rtm_id) {
                            ...
                            // Allocate rtm_coarse_cell_ids to M by N
                            rtm_coarse_cell_ids.insert(rtm_coarse_cell_ids.begin() + rtm_id_idx, vvint());
                            ...
                            for (auto const & coarse_cell_node : rtm_node.children("Grid")) {
                                ...
                                // If the coarse cell ID is not in coarse_cell_ids
                                auto coarse_cell_id_it = std::lower_bound(coarse_cell_ids.begin(), 
                                                                          coarse_cell_ids.end(), coarse_cell_id);
                                if (coarse_cell_id_it == coarse_cell_ids.end() || *coarse_cell_id_it != coarse_cell_id) {
                                    ptrdiff_t coarse_cell_id_idx = coarse_cell_id_it - coarse_cell_ids.begin();
                                    // Insert the ID to coarse_cell_ids
                                    coarse_cell_ids.insert(coarse_cell_ids.begin() + coarse_cell_id_idx, coarse_cell_id);
                                    // Read the mesh into a MeshFile object using read_xdmf_uniform_grid
                                    MeshFile<T, I> coarse_cell_mesh;
                                    coarse_cell_mesh.format = MeshFileFormat::XDMF;
                                    read_xdmf_uniform_grid(coarse_cell_node, h5file, h5filename, coarse_cell_mesh);
                                    // Set the coarse cell mesh type, mesh id, and material IDs
                                    MeshType const mesh_type = coarse_cell_mesh.get_mesh_type();
                                    coarse_cell_material_ids.insert(coarse_cell_material_ids.begin() + coarse_cell_id_idx,
                                                                    Vmatid());
                                    coarse_cell_mesh.get_material_ids(coarse_cell_material_ids[coarse_cell_id_idx],
                                                                      long_material_names);
                                    ...
                                    switch (mesh_type) {
                                        case MeshType::TRI:
                                            {
                                                coarse_cell_mesh_types.insert(coarse_cell_mesh_types.begin() + coarse_cell_id_idx,
                                                                              static_cast<I>(MeshType::TRI));
                                                coarse_cell_mesh_ids.insert(coarse_cell_mesh_ids.begin() + coarse_cell_id_idx,
                                                ...
                                                coarse_cell_dxdys[coarse_cell_id_idx] = Vec2d(static_cast<double>(dxdy[0]), 
    ...
    for (size_t i = 0; i < assembly_ids.size(); ++i)
    {
        ...
        UM2_ASSERT(std::is_sorted(assembly_lattice_zs[i].begin(), assembly_lattice_zs[i].end()));
