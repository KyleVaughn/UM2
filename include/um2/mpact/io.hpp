...
// TODO (kcvaughn@umich.edu): Split repeated code into functions. There's a lot of
// repeated code
...
writeXDMFFile(std::string const & path, mpact::SpatialPartition<T, I> const & model)
{
  ...
  for (Size iyasy = 0; iyasy < nyasy; ++iyasy) {
    for (Size ixasy = 0; ixasy < nxasy; ++ixasy) {
      ...
      for (Size izlat = 0; izlat < nzlat; ++izlat) {
        ...
        lat_found[static_cast<Size>(lat_id)] += 1;
        I const lat_id_ctr = lat_found[static_cast<Size>(lat_id)];
        ...
        for (Size iyrtm = 0; iyrtm < nyrtm; ++iyrtm) {
          for (Size ixrtm = 0; ixrtm < nxrtm; ++ixrtm) {
            I const rtm_id = lattice.getChild(ixrtm, iyrtm);
            rtm_found[static_cast<Size>(rtm_id)] += 1;
            I const rtm_id_ctr = rtm_found[static_cast<Size>(rtm_id)];
            ...
            for (Size iycell = 0; iycell < nycells; ++iycell) {
              for (Size ixcell = 0; ixcell < nxcells; ++ixcell) {
                I const cell_id = rtm.getChild(ixcell, iycell);
                cc_found[static_cast<Size>(cell_id)] += 1;
                AxisAlignedBox2<T> const cell_bb = rtm.getBox(ixcell, iycell);
                Point2<T> ll = asy_ll + rtm_ll + cell_bb.minima;
                I const cell_id_ctr = cc_found[static_cast<Size>(cell_id)];
                ...
                Size const num_mats = unique_mat_ids.size();
                std::sort(unique_mat_ids.begin(), unique_mat_ids.end());
                mesh_file.elset_names.resize(static_cast<size_t>(num_mats));
                mesh_file.elset_offsets.resize(static_cast<size_t>(num_mats) + 1);
                ...
                for (Size imat = 0; imat < num_mats; ++imat) {
                  ...
                  for (Size ielem = 0; ielem < static_cast<Size>(num_elements); ++ielem) {
                    if (mat_ids[ielem] == mat_id) {
                      ...
                      mesh_file.elset_offsets[static_cast<size_t>(imat) + 1U] = elem_ctr;
                    }
                  }
                }
                ...
                writeXDMFUniformGrid(xrtm_grid, h5file, h5filename, h5rtm_grouppath,
...
//   for (Size imat = 0; imat < material_names.size(); ++imat) {
...
//   // make_rtm,
//   //      and make_coarse_cell methods to create the model in order to avoid
//   duplicating
//   //      code. These functions take children IDs as arguments.
//   // - We have to create ID 1 before ID 2, ID 2 before ID 3, etc. since the make_*
//   methods
//   //      do not take its own ID as an argument. Therefore we have to construct the
//   model
//   //      in a bottom-up fashion, i.e. we have to create the coarse cells before we
//   can
//   //      create the RTMs, etc.
...
//   vvint assembly_lattice_ids;   // 1D map of the IDs of each lattice in each
//   assembly vvdouble assembly_lattice_zs; // Z coordinates of each lattice in each
//   assembly vint assembly_ids;            // IDs of all assemblies
//
//   vvvint lattice_rtm_ids; // 2D layout of the IDs of each RTM in each lattice
//   vint lattice_ids;       // IDs of all lattices
//
//   vvvint rtm_coarse_cell_ids; // 2D layout of the IDs of each coarse cell in each
//   RTM vint rtm_ids;               // IDs of all RTMs
//
...
//       vint()); assembly_lattice_ids[assembly_id_idx].resize(assembly_M);
//       // Allocate assembly_lattice_zs to M + 1
...
//           vvint()); lattice_rtm_ids[lattice_id_idx].resize(lattice_M); for (auto &
//           lattice_rtm_id : lattice_rtm_ids[lattice_id_idx]) {
...
//             }
//             // If the RTM ID is not in rtm_ids
//             auto rtm_id_it = std::lower_bound(rtm_ids.begin(), rtm_ids.end(),
//             rtm_id); if (rtm_id_it == rtm_ids.end() || *rtm_id_it != rtm_id) {
...
//                 // Extract the coarse cell ID from the name
//                 // Of the form Coarse_Cell_XXXXX_YYYYY, where XXXXX is the coarse
//                 cell ID std::string coarse_cell_name =
...
//                       coarse_cell_material_ids.begin() + coarse_cell_id_idx,
//                       Vmatid());
