...
public:
  ...
  using Core = RectilinearPartition2<T, I>;
...
private:
  ...
  Vector<Material<T>> _materials;

  Vector<TriMesh<2, T, I>> _tri;
  Vector<QuadMesh<2, T, I>> _quad;
  Vector<QuadraticTriMesh<2, T, I>> _quadratic_tri;
  Vector<QuadraticQuadMesh<2, T, I>> _quadratic_quad;
...
template <std::floating_point T, std::integral I>
PURE [[nodiscard]] constexpr auto
SpatialPartition<T, I>::getCoarseCell(Size cc_id) const noexcept -> CoarseCell const &
{
...
template <std::floating_point T, std::integral I>
PURE [[nodiscard]] constexpr auto
SpatialPartition<T, I>::getRTM(Size rtm_id) const noexcept -> RTM const &
{
  return _rtms[rtm_id];
}
...
template <std::floating_point T, std::integral I>
PURE [[nodiscard]] constexpr auto
SpatialPartition<T, I>::getLattice(Size lat_id) const noexcept -> Lattice const &
{
...
template <std::floating_point T, std::integral I>
PURE [[nodiscard]] constexpr auto
SpatialPartition<T, I>::getAssembly(Size asy_id) const noexcept -> Assembly const &
{
...
SpatialPartition<T, I>::makeCoarseCell(Vec2<T> const dxdy, MeshType const mesh_type,
                                       ...
                                       Vector<MaterialID> const & material_ids) -> Size
{
  Size const cc_id = _coarse_cells.size();
...
SpatialPartition<T, I>::makeRTM(Vector<Vector<Size>> const & cc_ids) -> Size
{
  ...
  RectilinearGrid2<T> const grid(dxdy, cc_ids_renumbered);
...
SpatialPartition<T, I>::makeLattice(Vector<Vector<Size>> const & rtm_ids) -> Size
{
  ...
  RegularGrid2<T> const grid(minima, spacing, num_cells);
    ...
    -> Size
{
  ...
  Size const num_lat = _lattices.size();
  {
    auto const * const it =
...
//                         cc_kns_max[cell_id].push_back(static_cast<T>(1) / (t_max *
//                         mcl)); cc_kns_mean[cell_id].push_back(static_cast<T>(1) /
//                                                        (t_mean * mcl));
//                       }
//                     }
...
//                           T const t_mean = materials[mat_id].xs.getOneGroupTotalXS(
//                               XSReductionStrategy::Mean);
//                           cc_kns_max[cell_id][iface] = static_cast<T>(1) / (t_max *
