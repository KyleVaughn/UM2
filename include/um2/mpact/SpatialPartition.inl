...
void SpatialPartition<T, I>::clear() {
    this->core.clear();
...
//  if (num_azimuthal < 8)
//  {
//    Log::error("The number of azimuthal divisions must be at least 8 until someone fixes the AABB mesh for 4 divisions");
//  }
//  if (radii.size() != num_rings.size())
//  {
//    Log::error("The number of radii must match the size of the subradial rings vector");
//  }
...
//  }
//
//  // radial region = region containing different materials (rings + outside of the last radius)
//  //   radial_region_areas = area of each radial region, including outside of the last radius
//  // ring = an equal area division of a radial region containing the same material
//  //   ring_radii = the radius of each ring, NOT including the outside of the last radius
//  //   ring_areas = the area of each ring, including the outside of the last radius
//  length_t mesh_id = -1;
//  if (mesh_order == 1) {
//    mesh_id = this->quad.size();
//    Log::info("Making linear quadrilateral cylindrical pin mesh " + std::to_string(mesh_id));
//  } else if (mesh_order == 2) {
//    mesh_id = this->quadratic_quad.size();
//    Log::info("Making quadratic quadrilateral cylindrical pin mesh " + std::to_string(mesh_id));
//  } else {
...
//  std::vector<double> radial_region_areas(num_radial_regions); // Each of the radii + the pin pitch
...
//  }
//  radial_region_areas[num_radial_regions - 1] = pitch * pitch - radial_region_areas[num_radial_regions - 2];
//
...
//    ring_radii[i] = std::sqrt(ring_areas[i] / pi + ring_radii[i - 1] * ring_radii[i - 1]);
//  }
...
//      ring_radii[ctr] = std::sqrt(area_per_ring / pi + ring_radii[ctr - 1] * ring_radii[ctr - 1]);
//    }
//  }
...
//  {
//    Log::debug("Ring " + std::to_string(i) + " radius: " + std::to_string(ring_radii[i]));
//    Log::debug("Ring " + std::to_string(i) + " area: " + std::to_string(ring_areas[i]));
//  }
//  Log::debug("The area outside of the last ring is " + std::to_string(ring_areas[ctr]));
...
//    {
//      eq_radii[i] = sqrt(2 * ring_areas[i] / (sin_theta * num_azimuthal) + eq_radii[i - 1] * eq_radii[i - 1]);
//    }
//    // Log the equivalent radii in debug mode
//    for (size_t i = 0; i < total_rings; ++i)
//    {
//      Log::debug("Ring " + std::to_string(i) + " equivalent radius: " + std::to_string(eq_radii[i]));
//    }
//    // If any of the equivalent radii are larger than half the pitch, error
//    if (std::any_of(eq_radii.begin(), eq_radii.end(), [pitch](double r) { return r > pitch / 2; }))
//    {
//      Log::error("The equivalent radius of a ring is larger than half the pitch.");
//      return -1;
//    }
//    // Sanity check: ensure the sum of the quadrilateral areas in a ring is equal to the ring area
//    UM2_ASSERT(std::abs(eq_radii[0] * eq_radii[0] * sin_theta / 2 - ring_areas[0] / num_azimuthal) < 1e-4)
//    for (size_t i = 0; i < total_rings; ++i)
//    {
//      double const area = (eq_radii[i] * eq_radii[i] - eq_radii[i - 1] * eq_radii[i - 1]) * sin_theta / 2;
//      UM2_ASSERT(std::abs(area - ring_areas[i] / num_azimuthal) < 1e-4)
//    }
//
...
//    //   Center point
//    //   num_azimuthal / 2, for the points in the innermost ring to make the quads "triangular"
//    //   (num_rings + 1) * num_azimuthal
//    size_t const num_points = 1 + (total_rings + 1) * num_azimuthal + num_azimuthal / 2;
...
//          cos_ia_theta = 0;
//        }
//        vertices[num_prev_points + ia] = {eq_radii[ir] * cos_ia_theta, eq_radii[ir] * sin_ia_theta};
//      }
//    }
...
//    {
//      Log::debug("Point " + std::to_string(i) + ": " + std::to_string(vertices[i][0]) + ", " + std::to_string(vertices[i][1]));
//    } 
//
...
//    //        = (4 / 3) * 2l sin(θ/2) * (L - l cos(θ/2)) / 2 = (4 / 3) * l sin(θ/2) * (L - l cos(θ/2))
//    // 
//    // For N + 1 rings
//    // A_0 = pi R_0² = Na ( A_t + A_e0)
//    // A_i = pi (R_i² - R_{i-1}²) = Na ( A_q + A_ei - A_ei-1)
//    // A_N = P² - pi R_N² = P² - sum_i=0^N A_i
//    // Constraining L_N to be the value which minimizes the 2-norm of the integral of the quadratic segment
//    // minus the circle arc is the correct thing to do, but holy cow the integral is a mess. 
...
//      double const A_edge_im1 = (4.0 / 3.0) * l_im1 * sin_gamma * (L_im1 - l_im1 * cos_gamma);
//      double const l = ring_radii[i];
//      double const A_quad = (l * l - l_im1 * l_im1) * sincos_gamma;
//      double const A_ring = ring_areas[i] / num_azimuthal;
//      eq_radii[i] = 0.75 * (A_ring - A_quad + A_edge_im1) / (l * sin_gamma) + l * cos_gamma;
//    }
//    // Log the equivalent radii in debug mode
//    for (size_t i = 0; i < total_rings; ++i)
//    {
...
//      // if ia is 0 or even, just do the 1 center point, otherwise we need 3 points at (1/4, 2/4, 3/4) of the radius
//      if (ia % 2 == 0)
//      {
...
//        cos_ia_gamma = 0;
//      }
//      // if ia is 0 or even, we want the point at ring_radii[ir], otherwise we want the point at eq_radii[ir]
//      if (ia % 2 == 0)
//      {
//        vertices[num_prev_points + ia] = {ring_radii[0] * cos_ia_gamma, ring_radii[0] * sin_ia_gamma};
//      }
//      else
//      {
//        vertices[num_prev_points + ia] = {eq_radii[0] * cos_ia_gamma, eq_radii[0] * sin_ia_gamma};
//      }
//    }
...
//        if (ia % 2 == 0)
//        {
//          vertices[num_prev_points + ia] = {ring_radii[ir] * cos_ia_gamma, ring_radii[ir] * sin_ia_gamma};
//        }
//        else
//        {
//          vertices[num_prev_points + ia] = {eq_radii[ir] * cos_ia_gamma, eq_radii[ir] * sin_ia_gamma};
//        }
//      }
//    }
...
//    for (size_t i = 0; i < num_faces; ++i)
//    {
//      Log::debug("Face " + std::to_string(i) + ": " + std::to_string(faces[i][0]) + ", " + std::to_string(faces[i][1]) + ", " + std::to_string(faces[i][2]) + ", " + std::to_string(faces[i][3]) + ", " + std::to_string(faces[i][4]) + ", " + std::to_string(faces[i][5]) + ", " + std::to_string(faces[i][6]) + ", " + std::to_string(faces[i][7]));
//    }
//
//    Vector<Point2<T>> vertices_t(static_cast<length_t>(num_points)); 
//    double const half_pitch = pitch / 2;
...
//                                             I const mesh_id,           
//                                             std::vector<Material> const & cc_materials)
//{
...
//                    Log::error("Tri-quad mesh " + std::to_string(mesh_id) + " does not exist");
//                    return -1;
//                }
...
//                UM2_ASSERT(cc_materials.size() == num_faces(this->quadratic_tri[mesh_id]));
//                break;
//            case 5:
...
//                UM2_ASSERT(cc_materials.size() == num_faces(this->quadratic_quad[mesh_id]));
//                break;
//            case 6:   
//                if (!(0 <= mesh_id && mesh_id < this->quadratic_tri_quad.size())) {
//                    Log::error("Quadratic tri-quad mesh " + std::to_string(mesh_id) + " does not exist");
//                    return -1;
//                }
//                aabb = bounding_box(this->quadratic_tri_quad[mesh_id].vertices);
//                UM2_ASSERT(cc_materials.size() == num_faces(this->quadratic_tri_quad[mesh_id]));
//                break;
...
//        auto it = std::find(this->materials.begin(), this->materials.end(), cc_materials[i]);
//        ptrdiff_t const idx = std::distance(this->materials.begin(), it);
//        if (idx >= this->materials.size()) {
...
//    // To get around this requirement, we will renumber the coarse cells to be 0, 1, 2, 3, ...
...
//            UM2_ASSERT(it != unique_cc_ids.end());
...
//        double const assem_top = static_cast<double>(this->assemblies[0].grid.divs[0].back());
//        double const assem_bot = static_cast<double>(this->assemblies[0].grid.divs[0].front());
//        if (std::abs(z.back() - assem_top) > eps ||
...
//        }
//        for (size_t j = 0; j < num_cols; ++j) {
//            ass_ids_flat[i * num_cols + j] = static_cast<I>(ass_ids[num_rows - 1 - i][j]);
//        }
//    }
//    this->core.grid = grid;
...
//                    // Shift the points so that the min point is at the origin.
//                    QuadraticTriQuadMesh<T, I> & mesh = this->quadratic_tri_quad.back();
//                    AABox2<T> bb = bounding_box(mesh);
//                    Point2<T> min_point = bb.minima;
//                    for (auto & p : mesh.vertices) { p -= min_point; }
//                    Point2<T> dxdy = bb.maxima - bb.minima;
...
//
//template <std::floating_point T, std::signed_integral I>
//void SpatialPartition<T, I>::coarse_cell_heights(Vector<std::pair<int, double>> & id_dz) const
//{
...
//                      if (id_dz[i].first == cc_id && std::abs(id_dz[i].second - dz) < 1e-4) {
//                        add_id = false;
//                        break;
//                      }
//                    }
...
//                return std::abs(p1.second - p2.second) < 1e-4 ? p1.first < p2.first : p1.second < p2.second;
//            });
//}
//
//template <std::floating_point T, std::signed_integral I>
//void SpatialPartition<T, I>::coarse_cell_face_areas(length_t const cc_id, Vector<T> & areas) const
//{
...
//
//template <std::floating_point T, std::signed_integral I>
//void SpatialPartition<T, I>::lattice_heights(Vector<std::pair<int, double>> & id_dz) const
//{
//    // For each unique assembly
//    //  For each lattice in the assembly
//    //    Get the dz of the lattice
...
//            *vertices = reinterpret_cast<T *>(this->tri_quad[mesh_id].vertices.begin());
//            *fv_offsets = this->tri_quad[mesh_id].fv_offsets.begin();
//            *fv = this->tri_quad[mesh_id].fv.begin();
//            break;
//        case static_cast<I>(MeshType::QUADRATIC_TRI):
...
//            *vertices = reinterpret_cast<T *>(this->quadratic_tri[mesh_id].vertices.begin());
//            *fv_offsets = nullptr;
...
//            *vertices = reinterpret_cast<T *>(this->quadratic_quad[mesh_id].vertices.begin());
//            *fv_offsets = nullptr;
...
//            *vertices = reinterpret_cast<T *>(this->quadratic_tri_quad[mesh_id].vertices.begin());
//            *fv_offsets = this->quadratic_tri_quad[mesh_id].fv_offsets.begin();
//            *fv = this->quadratic_tri_quad[mesh_id].fv.begin();
//            break;
//        default:
//            Log::error("Mesh type not supported");
//    }
