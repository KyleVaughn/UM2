...
public:
  ...
  [[nodiscard]] auto
  getElementBoundingBox(Size i) const -> AxisAlignedBox3<T>;

  [[nodiscard]] auto
  getElementCentroid(Size i) const -> Point3<T>;
...
template <std::floating_point T, std::signed_integral I>
auto
PolytopeSoup<T, I>::getElementBoundingBox(Size const i) const -> AxisAlignedBox3<T>
{
  LOG_TRACE("PolytopeSoup::getElementBoundingBox(i = " + toString(i) + ")");
  ASSERT(i < _element_types.size());

  VTKElemType const elem_type = _element_types[i];
  Size const istart = static_cast<Size>(_element_offsets[i]);
  ...
  case VTKElemType::Line: {
    ...
    LineSegment<3, T> const line(p0, p1);
  ...
  case VTKElemType::Triangle: {
    auto const p0 = _vertices[static_cast<Size>(_element_conn[istart])];
    auto const p1 = _vertices[static_cast<Size>(_element_conn[istart + 1])];
    auto const p2 = _vertices[static_cast<Size>(_element_conn[istart + 2])];
    Triangle3<T> const tri(p0, p1, p2);
    box = boundingBox(tri);
    break;
  }
  case VTKElemType::Quad: {
    Vec<4, Point2<T>> p2;
    for (Size j = 0; j < 4; ++j) {
      auto const p = _vertices[static_cast<Size>(_element_conn[istart + j])];
      p2[j][0] = p[0];
      p2[j][1] = p[1];
      ASSERT_NEAR(p[2], z, eps_distance<T>);
    }
...
template <std::floating_point T, std::signed_integral I>
auto
PolytopeSoup<T, I>::getElementCentroid(Size const i) const -> Point3<T>
{
  ...
  switch (elem_type) {
  case VTKElemType::Vertex:
    c = _vertices[static_cast<Size>(_element_conn[istart])];
    break;
  case VTKElemType::Line: {
    auto const p0 = _vertices[static_cast<Size>(_element_conn[istart])];
    auto const p1 = _vertices[static_cast<Size>(_element_conn[istart + 1])];
...
PolytopeSoup<T, I>::mortonSortElements()
{
  ...
  Vector<uint32_t> morton_codes(num_elems, 0);
  for (Size i = 0; i < num_elems; ++i) {
  ...
  for (Size i = 0; i < num_elems; ++i) {
    ...
    for (I j = 0; j < num_verts; ++j) {
      new_conn[static_cast<Size>(new_offsets[i] + j)] =
          _element_conn[static_cast<Size>(old_offset_start + j)];
    }
  }
...
template <std::floating_point T, std::signed_integral I>
void
PolytopeSoup<T, I>::reserveMoreElements(VTKElemType const elem_type, Size const num_elems)
{
  // Element types
...
template <std::floating_point T, std::signed_integral I>
void
PolytopeSoup<T, I>::reserveMoreVertices(Size const num_verts)
{
