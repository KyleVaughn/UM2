...
QuadraticQuadMesh<D, T, I>::FaceVertexMesh(MeshFile<T, I> const & file)
{
  ...
  if (verts_per_face != 8) {
    Log::error("Attempted to construct a QuadraticQuadMesh from a mesh file with "
               "non-quad8 faces");
  }
  ...
  for (Size i = 0; i < num_faces; ++i) {
    ...
    if (Quadrilateral2<T>(v0, v1, v2, v3).area() < 0) {
      ...
      std::swap(vi1, vi3);
      std::swap(face[4], face[7]);
      std::swap(face[5], face[6]);
    }
  }
#endif
}
...
template <Size D, std::floating_point T, std::signed_integral I>
PURE HOSTDEV constexpr auto
QuadraticQuadMesh<D, T, I>::face(Size i) const noexcept -> Face
{
...
QuadraticQuadMesh<D, T, I>::toMeshFile(MeshFile<T, I> & file) const noexcept
{
  ...
  file.type = MeshType::QuadraticQuad;
  Size const n = 8;
