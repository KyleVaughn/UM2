...
QuadMesh<D, T, I>::FaceVertexMesh(MeshFile<T, I> const & file)
{
  ...
  if (verts_per_face != 4) {
    Log::error("Attempted to construct a QuadMesh from a mesh file with non-quad faces");
  }
  ...
  for (Size i = 0; i < num_faces; ++i) {
    ...
    Point2<T> const & v3 = this->vertices[static_cast<Size>(vi3)];
    if (Quadrilateral2<T>(v0, v1, v2, v3).area() < 0) {
...
template <Size D, std::floating_point T, std::signed_integral I>
PURE HOSTDEV constexpr auto
QuadMesh<D, T, I>::face(Size i) const noexcept -> Face
{
  ...
  return Quadrilateral<D, T>(vertices[v0], vertices[v1], vertices[v2], vertices[v3]);
}
...
QuadMesh<D, T, I>::toMeshFile(MeshFile<T, I> & file) const noexcept
{
  ...
  auto const len = static_cast<size_t>(numFaces() * n);
