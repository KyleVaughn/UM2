// FINDINGS: The barycentric method is faster than the CCW method 
...
{
  return um2::areCCW(tri[0], tri[1], p) &&
         um2::areCCW(tri[1], tri[2], p) &&
         um2::areCCW(tri[2], tri[0], p);
}
// NOLINTEND(readability-identifier-naming)

static void containsBary(benchmark::State& state) {
...
static void containsCCW(benchmark::State& state) {
  um2::Vector<um2::Point<D, T>> points(static_cast<Size>(state.range(0)));
  um2::Vector<um2::Triangle<D, T>> tris(NTRIS);
  for (auto& p : points) {
    // cppcheck-suppress useStlAlgorithm
