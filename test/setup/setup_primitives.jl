# Setup geometry in the setup box for tests
...
function setup_QuadraticQuadrilateral2D(::Type{T}) where {T}
    return QuadraticQuadrilateral(Point2D{T}(0,0), Point2D{T}(1,0), 
                                  Point2D{T}(1,1), Point2D{T}(0,1),
                                  Point2D{T}(1//2, 0), Point2D{T}(9//10, 1//2), 
                                  Point2D{T}(1//2, 1), Point2D{T}(1//10, 1//2))
end

function setup_QuadraticQuadrilateral3D(::Type{T}) where {T}
    return QuadraticQuadrilateral(Point3D{T}(0,0,0), Point3D{T}(1,0,0), 
                                  Point3D{T}(1,1,0), Point3D{T}(0,1,0),
                                  Point3D{T}(1//2, 0, 1//2), 
