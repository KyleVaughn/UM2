...
main(int argc, char ** argv) -> int
{
  ...
  while (has_elset) {
    ...
    ids.clear();
    tmp.clear();
    um2::String const elset_name =
        "Homogenized_Total_XS_Group_" + um2::mpact::getASCIINumber(i);
    soup.getElset(elset_name, ids, tmp);
    if (!ids.empty()) {
      group_total_xs.push_back(tmp);
      has_elset = true;
    }
    um2::String const elset_name2 =
        "Homogenized_Scattering_Ratio_Group_" + um2::mpact::getASCIINumber(i);
    soup.getElset(elset_name2, ids, tmp);
    if (!ids.empty()) {
      group_scattering_ratio.push_back(tmp);
      has_elset = true;
    }
    um2::String const elset_name3 =
        "1D_odCMFD_Spectral_Radius_Group_" + um2::mpact::getASCIINumber(i);
  ...
  if (file == nullptr) {
    um2::logger::error("Could not open file ", total_xs_filename);
    return 1;
  }
  ...
  if (file == nullptr) {
    LOG_ERROR("Could not open file: ", scattering_ratio_filename);
    return 1;
  }
  ...
  if (file == nullptr) {
    LOG_ERROR("Could not open file: ", spectral_radius_filename);
    return 1;
  }
